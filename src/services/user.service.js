...
if (!JSON.parse(localStorage.getItem(USERS_KEY)))
...
async function removeLike(entity) {
  try {
    const loggedUser = getSessionUser();
    if (entity.type !== 'song') {
      var idx = loggedUser.liked[entity.type].findIndex(
        (currEntity) => entity._id === currEntity._id
      );
    } else {
      var idx = loggedUser.liked.song.findIndex(
        (song) => song.youtubeId === entity.youtubeId
      );
    }
...
async function removeFollow(follow) {
  try {
    const loggedUser = getSessionUser();
    const idx = loggedUser.follows.findIndex(
      (currFollow) => follow._id === currFollow._id
    );
